...
let users = [
  ...
  {
    id: 8,
    name: 'ZNicholas Runolfsdottir V',
...
/* 
    <div class="box">
        ...
        <h3>Leanne Graham</h3>
        <a href="mailto:Sincere@april.biz">Email</a>
        <button>
          <a href="tel:999999">Call me</a>
        </button>
    </div> 
*/
...
function render(users) {
  ...
  users.forEach((user) => {
    const { email, name, phone, edited, editedDate } = user
    const editedDateString = new Date(editedDate)
    ...
    const lastEditedDate = document.createElement('span')
    // lastEditedDate.textContent = editedDate
    //   ? editedDateString.getHours() + ' : ' + editedDateString.getMinutes()
    //   : ''
    lastEditedDate.textContent = editedDate
      ? formatter.format(
          -(new Date().getMinutes() - editedDateString.getMinutes()) || 0,
          'minute'
        )
      : null
    editIcon.textContent = edited ? '✍️' : null
    ...
    box.appendChild(lastEditedDate)
